...
export function calculateEligibility(inputs: LoanInputs): EligibilityResult {
  ...
  if (inputs.creditScore >= 750) score += 30;
  else if (inputs.creditScore >= 700) score += 20;
  else if (inputs.creditScore >= 650) score += 10;
  else if (inputs.creditScore >= 600) score += 0;
  else score -= 15;

  // EMI-to-income ratio
  const totalEMI = inputs.outstandingEMI + (inputs.hasCreditCard ? inputs.creditCardOutstanding * 0.05 : 0);
  const emiToIncome = inputs.monthlyIncome > 0 ? (totalEMI / inputs.monthlyIncome) * 100 : 100;

  if (emiToIncome < 20) score += 15;
  else if (emiToIncome < 40) score += 8;
  ...
  // Employment type
  if (inputs.employmentType === 'salaried') score += 5;
  else if (inputs.employmentType === 'self-employed') score += 0;
  else score -= 5;

  // Credit card history
  if (inputs.hasCreditCard && inputs.creditCardOutstanding < inputs.monthlyIncome * 0.3) score += 5;

  // Loan enquiries
  if (inputs.loanEnquiries <= 1) score += 5;
  else if (inputs.loanEnquiries <= 3) score += 0;
  else score -= 10;
  ...
  const maxEligibleAmount = Math.max(0, Math.round((maxEMI * 12 * 5) / (1 + (baseRate / 100) * 5)));
  const monthlyEMIEstimate = inputs.loanAmountNeeded > 0
    ? Math.round((inputs.loanAmountNeeded * (1 + (baseRate / 100) * 5)) / (5 * 12))
    : 0;
...
export function getSampleLenders(result: EligibilityResult, loanType: string): LenderMatch[] {
  ...
  return lenders.filter((_, i) => i < (result.score >= 70 ? 4 : result.score >= 50 ? 3 : 2));
}
