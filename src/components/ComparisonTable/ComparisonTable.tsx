  ...
  return (
    <div className="overflow-x-auto rounded-2xl border border-violet-300 shadow-sm">
      <table className="w-full border-collapse">
          ...
          <tr className="bg-gradient-to-r from-[#140c24] to-[#1f1338] text-white">
            ...
            <th className="px-4 py-4 text-left font-semibold">
              <div className="flex items-center gap-2">
                ...
                {customAmount ? "Requested Amount" : "Max Amount"}
              </div>
            </th>
        ...
        <tbody>
          {lenders.map((lender) => {
            const displayAmount = customAmount ?? lender.maxSanctionAmount;
            return (
              <tr
                key={lender.id}
                onClick={() =>
                  !isDisabled(lender.id) && onSelectLender(lender.id)
                }
                className={cn(
              ...
              >
                ...
                <td className="px-4 py-4 font-medium">
                  ₹{new Intl.NumberFormat("en-IN").format(displayAmount)}
                </td>

                <td className="px-4 py-4 font-medium">{lender.trueAPR}%</td>
